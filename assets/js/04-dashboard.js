...
function renderPendingPayments(){
  ...
  if(!role){panel.innerHTML='';return;}
  const entries=Object.entries(tableOrders).filter(([,t])=>['pending_payment','waiting_verification','paid'].includes(t.status)).sort((a,b)=>(b[1]?.claimedPaidAt||b[1]?.createdAt||0)-(a[1]?.claimedPaidAt||a[1]?.createdAt||0));
  if(!entries.length){panel.innerHTML='';return;}
  panel.innerHTML=`<div class="pending-panel-wrap"><div class="pending-card"><div class="pending-ph"><span class="pending-ph-title">Pesanan &amp; Pembayaran</span><span class="pending-badge">${entries.filter(([,t])=>['pending_payment','waiting_verification'].includes(t.status)).length}</span></div>${entries.map(([tid,t])=>{const state=t.status==='paid'?'Sudah dikonfirmasi':t.status==='pending_payment'?'Menunggu diproses kasir':'Tamu mengaku sudah bayar';const stateColor=t.status==='paid'?'#5fa97c':t.status==='pending_payment'?'#20c8c8':'var(--gold)';const action=['pending_payment','waiting_verification'].includes(t.status)?`<button class="konfirm-btn" data-tid="${tid}">Konfirmasi Bayar</button>`:`<button class="clear-btn" data-tid="${tid}" style="background:none;border:1px solid #5fa97c;color:#5fa97c;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;font-family:'Outfit',sans-serif;transition:all 0.2s;flex-shrink:0">Selesai</button>`;
